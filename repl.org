...
#+begin_src clojure :tangle src/jakub_stastny/dev/repl.clj :mkdirp yes
  ...
  (defn contextual-eval [ctx expr]
    ...
    (prn
      `(let [~@(mapcat (fn [[k v]] [k `'~v]) ctx)]
          ~expr))
    (super-unsafe-eval ; This won't work in CLJS I think.
      `(let [~@(mapcat (fn [[k v]] [k `'~v]) ctx)]
          ~expr)))
  ...
  (defn knievel [expr]
    (prn :eval :expr expr :ctx (local-context))
    ;; (eval expr)
    ;:done ; Just return

    ; :eval :expr i :ctx {expr i}
    ; Should this have i as a key also? Or that's sorted in the eval?

    ; We're evaluating (clojure.core/let [expr (quote i)] i)

    ; THE ISSUE IS IT NEVER EVEN TRIES TO EVALUATE IN contextual-eval, it fails before that.
    ; Unable to resolve symbol: i in this context
    (contextual-eval (local-context) expr)
    )

  ;; (def knievel (partial contextual-eval (local-context)))

  (defn prompt [] (print "debug=> "))

  (defmacro break []
    `(clojure.main/repl :prompt prompt :eval knievel))
